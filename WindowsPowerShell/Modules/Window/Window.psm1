...
class ScrollView {
	...
	[void] OnKey([System.ConsoleKeyInfo] $key) {
		...
		[Log]::Comment("SV.OnKey: $($key.Key) $($key.Modifiers)")
	...
	[WindowResult] Run() {
		...
		$Global:Host.UI.RawUI.SetBufferContents($this.LeftTop(), $this._originalBufferContent)
		return $this.Result
	}
	...
	hidden [void] DrawFrame() {
		...
		$Global:Host.UI.RawUI.SetBufferContents($this.LeftTop(), $buffer)
	}
	...
	[System.Management.Automation.Host.Coordinates] LeftTop() {
